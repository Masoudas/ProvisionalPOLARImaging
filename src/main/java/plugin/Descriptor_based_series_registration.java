...
public class Descriptor_based_series_registration implements PlugIn
{
	...
	public void run( final String arg0 ) 
	{
		...
		if ( reApply )
		{
			...
			POLARMatching.descriptorBasedStackRegistration( imp, params );
			return;
		}
	...
	public static String[] globalOptTypes = { "All-to-all POLARMatching (global optimization)", "All-to-all POLARMatching with range ('reasonable' global optimization)", "All against first image (no global optimization)", "Consecutive POLARMatching of images (no global optimization)" };
	...
	protected DescriptorParameters getParameters( final ImagePlus imp, final int dimensionality )
	{
		...
		gd.addSlider( "Range for all-to-all POLARMatching", 2, 10, defaultRange );
			 ...
			 detectionTypeIndex == detectionTypes.length - 1 )
		{
			...
			interactiveTmp.show();
			final InteractiveDoG idog = POLARRegistration.getInteractiveDoGParameters( interactiveTmp, 1, values, 20 );
		...
		else 
		{
			if ( detectionBrightnessIndex == detectionBrightness.length - 2 || detectionSizeIndex == detectionSize.length - 2 )
			{
				...
				final double[] values = POLARRegistration.getAdvancedDoGParameters( defaultSigma, defaultThreshold );
				params.sigma1 = values[ 0 ];
				params.threshold = values[ 1 ];				
			}
			else
