...
public class POLARRegistration implements PlugIn 
{
	...
	public static int defaultImg3 = 2;
	public static int defaultImg4 = 3;
	...
	{		
		...
		{
			IJ.error( "You need four open images." );
			return;
		}
		...
		gd.addChoice("Channel 0-degree image", imgList, imgList[ defaultImg1 ] );
		gd.addChoice("Channel 45-degree image", imgList, imgList[ defaultImg2 ] );
		gd.addChoice("Channel 90-degree image", imgList, imgList[ defaultImg3 ] );
		gd.addChoice("Channel 135-degree image", imgList, imgList[ defaultImg4 ] );
		...
		ImagePlus[] imp = {
			WindowManager.getImage( idList[ defaultImg1 = gd.getNextChoiceIndex() ] ),
			WindowManager.getImage( idList[ defaultImg2 = gd.getNextChoiceIndex() ] ),
			WindowManager.getImage( idList[ defaultImg3 = gd.getNextChoiceIndex() ] ),
			WindowManager.getImage( idList[ defaultImg3 = gd.getNextChoiceIndex() ] ),		
		};
		...
		//ImagePlus imp3 = WindowManager.getImage( idList[ defaultImg3 = gd.getNextChoiceIndex() ] );		
		//ImagePlus imp4 = WindowManager.getImage( idList[ defaultImg3 = gd.getNextChoiceIndex() ] );		
		ImagePlus imp1, imp2;
		DescriptorParameters params = null;
		boolean reApply = false;
		final int[] polarizations = {35, 90, 135};
		...
		imp1 = Hyperstack_rearranger.convertToHyperStack( imp[0] );
		for (int imCounter = 1; imCounter < 4; imCounter++){
			// if one of the images is rgb or 8-bit color convert them to hyperstack
			
			imp2 = Hyperstack_rearranger.convertToHyperStack( imp[imCounter] );
			...
			if ( reApply )
			{
				...
				// just fuse
				params = new DescriptorParameters();
				params.dimensionality = dimensionality;
				params.reApply = true;
				params.fuse = 0;
				params.setPointsRois = false;
				POLARMatching.descriptorBasedRegistration( imp1, imp2, params, polarizations[imCounter-1] );
				return;			
			}

			// open a second dialog and query the other parameters
			//if (imCounter == 1){
			params = getParameters( imp1, imp2, dimensionality );
			//}

			if ( params == null )
				return;
					
			// compute the actual POLARMatching
			POLARMatching.descriptorBasedRegistration( imp1, imp2, params, polarizations[imCounter-1] );
	...
	public String[] transformationModels2d = new String[] { "Affine (2d)"};
	public String[] transformationModels3d = new String[] { "Affine (3d)" };
	public static int defaultTransformationModel = 0;
	public static int defaultRegularizationTransformationModel = 0;
	...
	protected DescriptorParameters getParameters( final ImagePlus imp1, final ImagePlus imp2, final int dimensionality )
	{
		...
		gd.addChoice( "Transformation_model", transformationModel, transformationModel[ 0 ] );
		...
		gd.addSlider( "Redundancy for descriptor POLARMatching", 0, 10, defaultRedundancy );		
		...
		final int transformationModelIndex = 3;//gd.getNextChoiceIndex();
		...
		if ( similarOrientation == 2 )
		{
			...
			{
				IJ.log( "No parameters necessary ... the POLARMatching is anyways translation invariant." );
			}
